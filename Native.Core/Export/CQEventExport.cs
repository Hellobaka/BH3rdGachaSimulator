...
namespace Native.App.Export
{
	...
	{	
		...
		{	
			return "9,me.cqp.luohuaming.Gacha";	
		}	
		...
		{	
			...
			Type appDataType = typeof (AppData);	
			// 注册一个 CQApi 实例	
			AppInfo appInfo = new AppInfo ("me.cqp.luohuaming.Gacha", 1, 9, "水银崩坏三抽卡模拟", "1.3.4", 1, "落花茗", "提供崩坏三抽卡模拟(V9应用机制)", authCode);	
			appDataType.GetRuntimeProperty ("CQApi").GetSetMethod (true).Invoke (null, new object[] { new CQApi (appInfo) });	
			AppData.UnityContainer.RegisterInstance<CQApi> ("me.cqp.luohuaming.Gacha", AppData.CQApi);	
			// 向容器注册一个 CQLog 实例	
			appDataType.GetRuntimeProperty ("CQLog").GetSetMethod (true).Invoke (null, new object[] { new CQLog (authCode) });	
			AppData.UnityContainer.RegisterInstance<CQLog> ("me.cqp.luohuaming.Gacha", AppData.CQLog);	
